...
BBox3D Plane::bbox() const {
	return BBox3D(vec3(std::numeric_limits<double>::infinity()),
...
HitInfo Plane::sample(const vec3& localPos, const Affine& transform, float& pdf) const {
	throw std::runtime_error("Cannot sample inifinite plane");
}
float Plane::pdf(const vec3& pos, const vec3& wi, const Affine& transform, const Scene* scene) const {
	return 0.0f;
}
